...
export default async function handler(request: VercelRequest<InviteBody>, response: VercelResponse) {
  ...
  try {
        ...
        <div style="margin:0;padding:32px;background:#f6f8fb;font-family:Arial,sans-serif;color:#111827">
          <div style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden">
            <div style="padding:28px 32px;background:#0f172a;color:#ffffff">
              <h1 style="margin:0;font-size:22px;line-height:1.3">You're invited to ${companyName}</h1>
            </div>
            <div style="padding:28px 32px">
              <p style="margin:0 0 16px;font-size:15px;line-height:1.6">${inviterName} invited you to join the ${companyName} workspace.</p>
              ...
              <a href="${inviteLink}" style="display:inline-block;background:#2563eb;color:#ffffff;text-decoration:none;font-weight:700;padding:12px 18px;border-radius:8px">Accept Invitation</a>
    ...
    if (error) {
      response.status(502).json({ error: error.message || 'Failed to send invitation email' });
      return;
    }
